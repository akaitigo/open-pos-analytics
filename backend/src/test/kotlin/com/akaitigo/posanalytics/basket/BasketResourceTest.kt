...
class BasketResourceTest {
    ...
    fun `sort=support で最頻併売ペアが先頭に来る`() {
        given().queryParam("sort", "support").`when`().get(PAIRS)
            .then().statusCode(OK)
    ...
    fun `segment=morning はその時間帯のペアだけを返す`() {
        given().queryParam("segment", "morning").`when`().get(PAIRS)
            .then().statusCode(OK)
    ...
    fun `segment=evening はその時間帯のペアだけを返す`() {
        given().queryParam("segment", "evening").`when`().get(PAIRS)
            .then().statusCode(OK)
            .body("pairs.size()", equalTo(1))
    ...
    fun `商品コードに対応する商品名が解決される`() {
        given().queryParam("segment", "noon").queryParam("sort", "support").`when`().get(PAIRS)
            .then().statusCode(OK)
    ...
    @Test
    fun `limit で件数を制限できる`() {
        given().queryParam("limit", "1").`when`().get(PAIRS)
            .then().statusCode(OK)
    ...
    fun `limit の境界値 1 と 100 は 200`() {
        given().queryParam("limit", "1").`when`().get(PAIRS).then().statusCode(OK)
        given().queryParam("limit", "100").`when`().get(PAIRS).then().statusCode(OK)
    }

    @Test
    fun `不正な sort は 400 を返す`() {
        given().queryParam("sort", "price").`when`().get(PAIRS)
    ...
    fun `不正な segment は 400 を返す`() {
        given().queryParam("segment", "midnight").`when`().get(PAIRS)
            .then().statusCode(BAD_REQUEST)
    ...
    fun `limit が 0 は 400 を返す`() {
        given().queryParam("limit", "0").`when`().get(PAIRS)
            .then().statusCode(BAD_REQUEST)
    ...
    fun `limit が 101 は 400 を返す`() {
        given().queryParam("limit", "101").`when`().get(PAIRS)
            .then().statusCode(BAD_REQUEST)
            .body("field", equalTo("limit"))
    }
    ...
    fun `limit が非整数は 400 を返す`() {
        given().queryParam("limit", "abc").`when`().get(PAIRS)
            .then().statusCode(BAD_REQUEST)
            .body("field", equalTo("limit"))
    }
